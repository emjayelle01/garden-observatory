...
| `deploy/install-mgo-validate.sh` | **root**, run **directly** as `sudo ./scripts/deploy/install-mgo-validate.sh` — never `sudo bash …`, which discards the `#!/bin/bash -p` shebang and with it privileged mode. `--dry-run` needs no privilege | Installs the gateway (`0755 root:root`) and its sudoers policy (`0440 root:root`). Takes the shared deployment lock **first**, then copies both repository sources into its own workspace and validates, checksums and installs from that snapshot — so the bytes installed are the bytes validated even if a concurrent `deploy-main` moves the checkout. Validates with `/bin/bash -p -n` and `visudo -cf` both before installing and against what landed. Rolls both targets back on any failure, so the host never has half a control plane. Each run works in its own uniquely named workspace under `/run/mgo-validate-install` and removes only that; anything a previous run left behind is a **refusal** (exit 65), preserved for inspection, not a clean bill of health. Idempotent, but only when there is no stale transaction state. `--dry-run` is read-only, holds no lock, and its exit status agrees with its report. Never touches the approval file, the repository or `mgo.service`. |
