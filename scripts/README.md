...
| `deploy/install-mgo-validate.sh` | **root**, run **directly** as `sudo ./scripts/deploy/install-mgo-validate.sh` — never `sudo bash …`, which discards the `#!/bin/bash -p` shebang and with it privileged mode. `--dry-run` runs with or without privilege, and the two are not equivalent | Installs the gateway (`0755 root:root`) and its sudoers policy (`0440 root:root`). Takes the shared deployment lock **first**, then copies both repository sources into its own workspace and validates, checksums and installs from that snapshot — so the bytes installed are the bytes validated even if a concurrent `deploy-main` moves the checkout. Validates with `/bin/bash -p -n` and `visudo -cf` both before installing and against what landed. Rolls both targets back on any failure, so the host never has half a control plane. Each run works in its own uniquely named workspace under `/run/mgo-validate-install` and removes only that; anything a previous run left behind is a **refusal** (exit 65), preserved for inspection, not a clean bill of health. Idempotent, but only when there is no stale transaction state. `--dry-run` is read-only, holds no lock, and its exit status agrees with its report. Run unprivileged (`./scripts/deploy/install-mgo-validate.sh --dry-run`) it validates the **sources** this checkout would install and is the right staging check on any host; run as root (`sudo ./scripts/deploy/install-mgo-validate.sh --dry-run`) it is the authoritative pre-installation report. The unprivileged form is **not** a substitute: the installed targets are root-owned and `/etc/sudoers.d` is unreadable to an ordinary account, so it cannot distinguish a target that is already current from one it could not read, and it cannot re-validate the installed policy with `visudo`. Never touches the approval file, the repository or `mgo.service`. |
| `dev/run-mutations.py` | developer workstation | **Not an operator script.** Runs the deployment gateway's mutation register (`tests/mutation_register.py`) against the current tip: each entry is a deliberate defect in a shipped shell asset, applied to a byte-exact copy, tested, restored, and confirmed restored by digest. A contract nothing fails for is a contract nothing is enforcing. An entry that no longer applies fails as **stale** rather than passing quietly. Every candidate runs with its own bytecode state -- `-B`, `PYTHONDONTWRITEBYTECODE=1` and a fresh, task-owned `PYTHONPYCACHEPREFIX` per candidate, removed afterwards -- so two same-length mutations of one Python source inside one second cannot share stale bytecode and report a survivor for code that was never run (Task 14.5E). |
