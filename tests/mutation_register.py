...
MUTATIONS: tuple[Mutation, ...] = (
    ...
    # --- Task 14.2 operator safety correction round 1 ------------------------
    Mutation(
    ...
    Mutation(
        'a-wrongly-typed-configuration-value-becomes-an-internal-failure',
        RETENTION_CLI,
        '        TypeError,',
        '        RecursionError,',
        'a_malformed_configuration_is_an_operator_refusal',
        "An operator's own mistake is reported as an MGO defect.",
        suite=RETENTION_CLI_SUITE,
    ),
    Mutation(
        'a-wrongly-shaped-configuration-section-becomes-an-internal-failure',
        RETENTION_CLI,
        '        AttributeError,',
        '        RecursionError,',
        'a_malformed_configuration_is_an_operator_refusal',
        'A scalar where a table belongs is reported as an MGO defect.',
        suite=RETENTION_CLI_SUITE,
    ),
    Mutation(
        'the-destructive-gate-accepts-a-relative-configuration-path',
        RETENTION_CLI,
        '    if not Path(raw.strip()).is_absolute():',
        '    if False:',
        'a_relative_path_is_refused_even_when_it_would_resolve',
        'Which deployment gets deleted from depends on the working directory.',
        suite=RETENTION_CLI_SUITE,
    ),
    # --- Task 14.2 operator boundary correction round 2 ----------------------
    Mutation(
        'long-option-abbreviation-is-re-enabled',
        RETENTION_CLI,
        '        kwargs.setdefault("allow_abbrev", False)',
        '        kwargs.setdefault("allow_abbrev", True)',
        'an_abbreviated_execute_flag_is_refused',
        "'--exe' authorises a deletion, so a typo is a destructive consent.",
        suite=RETENTION_CLI_SUITE,
    ),
    Mutation(
        'argument-errors-revert-to-argparse-own-diagnostic',
        RETENTION_CLI,
        '        raise _Refusal(REFUSAL_INVALID_ARGUMENTS, EXIT_REFUSED)',
        '        super().error(message)',
        'an_invalid_invocation_never_echoes_the_operators_argument'
        ' or a_parser_refusal_reaches_the_injected_stream_and_no_other',
        "The operator's own arguments are echoed to the process stderr.",
        suite=RETENTION_CLI_SUITE,
    ),
    Mutation(
        'an-overflowing-configuration-value-becomes-an-internal-failure',
        RETENTION_CLI,
        '        OverflowError,',
        '        RecursionError,',
        'an_overflowing_configuration_value_is_an_operator_refusal',
        'Valid TOML with an infinite integer is reported as an MGO defect.',
        suite=RETENTION_CLI_SUITE,
    ),
    Mutation(
        'the-destructive-gate-expands-a-tilde-before-deciding',
        RETENTION_CLI,
        '    if not Path(raw.strip()).is_absolute():',
        '    if not Path(raw.strip()).expanduser().is_absolute():',
        'a_tilde_configuration_path_is_refused',
        'Which deployment gets deleted from depends on the executing account.',
        suite=RETENTION_CLI_SUITE,
    ),
)
...
TASK_14_5_MUTATIONS: tuple[Mutation, ...] = (
    ...
    Mutation(
        ...
        '            hourly = self._in_flight',
        'a_new_process_reconstructs_the_count_from_rows',
        'The hourly count is process-local and resets on restart.',
        suite=CAPTURE_ADMISSION_SUITE,
    ),
    Mutation(
        'reservation-ignored',
        ...
        ' or a_failed_capture_releases_its_reservation',
    ...
    Mutation(
        ...
        'Every identical suppression writes an observation.',
        suite=EVENT_CAPTURE_ADMISSION_SUITE,
    ),
    Mutation(
