...
keeps serving, published as a **three-file recovery set** (database snapshot,
production configuration snapshot, manifest) with checksum verification bound to
each artefact, isolated restore testing, bounded retention and a daily timer.
Production *restore* remains deliberately manual — `restore-test` proves
recoverability, and restoring over the live database is an explicit operator
procedure. Nothing in that tooling changes the schema, the migration files or any
