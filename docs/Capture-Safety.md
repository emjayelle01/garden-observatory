...
durable catalogue rows, so a restart cannot reset it; it probes the filesystem
the next JPEG would land on, so a full SD card stops it; and it fails closed
on every doubt, so a broken probe or an unreadable catalogue refuses rather
than admits. A refused trigger creates no image, no catalogue row and no
lifecycle row. An admitted capture that turns out larger than the reserved
size is withdrawn before it is catalogued. Separately, the motion detector no
longer mistakes an exposure step for a subject.
...
8. **reservation** — one in-flight slot is taken under the gate's lock.
...
an in-flight reservation counts as the capture it is about to become.
...
attempts, and every failure is a counter and an observation.

**Restart.** Nothing about the quota lives in memory except the reservation
for a capture executing right now, which cannot outlive the process because
the capture cannot either. A new process reconstructs the counts from rows on
its first evaluation, which the application performs once at startup so the
status endpoint reports real numbers rather than zeros.
...
is never touched. Together the reserve and the ceiling mean an admitted
...
last one recorded, and an admitted capture resets that memory. A windy
afternoon under quota therefore produces one timeline row, not hundreds.
`global_scene_change` writes nothing: the motion observer already persisted
the transition.
